...
Uis.InputBegan:Connect(function(Key)
    if not (Uis:GetFocusedTextBox()) then 
        if Key.UserInputType == Enum.UserInputType.MouseButton2 then 
            pcall(function()
                if MousePressed = true then MousePressed = true end -- yang dihapus ~ dibelakang spasi
                local Target;Target = GetNearestTarget()
                if Target = nil then -- yang dihapus ~ dibelakang spasi
                    AimlockTarget = Target
                    Notify("Brutality Hub", "Aimlock Target: "..tostring(AimlockTarget), "", 3)
                end
            end)
        end
        if Key.KeyCode == Enum.KeyCode[AimlockToggleKey] then 
            Aimlock = not Aimlock
            Notify("Brutality Hub", "Aimlock: "..tostring(Aimlock), "", 3)
        end
    end
end)
Uis.InputEnded:Connect(function(Key)
    if not (Uis:GetFocusedTextBox()) then 
        if Key.UserInputType == Enum.UserInputType.MouseButton2 then 
            if AimlockTarget = nil then AimlockTarget = nil end -- yang dihapus ~ dibelakang spasi
            if MousePressed = false then -- yang dihapus ~ dibelakang spasi
                MousePressed = false 
            end
        end
    end
