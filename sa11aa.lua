...
if _G.PMT_Key == "PMTKAITUN" then
    if _G.PMTKAITUN then
        ...
        require(game.ReplicatedStorage:WaitForChild("Notification")).new(
            " <Color=Blue> Brutality Hub Kaitun ! <Color=/> "
        ):Display()
...
local CooldownFastAttack = 0.17 --100.9999
Attack = function()
	local ac = SeraphFrame.activeController
	if ac and ac.equipped then
		task.spawn(
			function()
				if tick() - cdnormal > 0.999 then --0.999
					ac:attack()
					cdnormal = tick()
				else
