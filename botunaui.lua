do --// UI-Library \\-- 
    ...
    if getgenv().library then
        warn("球刀片.pub has already been executed")
        return;
    end
    
    local library = {design = "kali", tabs = {}, draggable = true, flags = {}, title = "", open = false, popup = nil, instances = {}, connections = {}, options = {}, notifications = {}, tabSize = 0, theme = {}, foldername = "Aztoop Hub V3", fileext = ".config.json"}
    ...
    function library:LoadConfig(config)
        if table.find(self:GetConfigs(), config) then
            ...
            writefile(string.format("%s/.json", library.foldername), game:GetService("HttpService"):JSONEncode({
                ["config"] = config
            }))
        end
    end
    ...
    library.createLabel = function(option, parent)
        option.main = library:Create("TextLabel", {
            ...
            TextSize = 15,
            Font = Enum.Font.Code,
            TextColor3 = option.color or Color3.new(1, 1, 1),
        ...
        setmetatable(option, {__newindex = function(t, i, v)
            if i == "Text" then
                ...
                option.main.Size = UDim2.new(1, -12, 0, 15)
            end
        end})
            ...
            if input.UserInputType.Name == "MouseMovement" then
                if not library.warning and not library.slider then
                    if option.style then
                        ...
                        --tweenService:Create(tickbox, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = library.flags["menuAccentColor"]}):Play()
                    else
                        tickbox.BorderColor3 = library.flags["menuAccentColor"]
                        tickboxOverlay.BorderColor3 = library.flags["menuAccentColor"]
                        --tweenService:Create(tickbox, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BorderColor3 = library.flags["menuAccentColor"]}):Play()
                        --tweenService:Create(tickboxOverlay, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BorderColor3 = library.flags["menuAccentColor"]}):Play()
                    end
                end
                if option.tip then
        ...
        function tab:AddColumn()
            ...
            function column:AddSection(title)
                ...
                function section:AddLabel(options)
                    local option = {text = options.text, color = options.color}
                    option.section = self
                    option.type = "label"
