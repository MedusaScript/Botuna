...
local Nurysium_Util = loadstring(game:HttpGet('https://pastebin.com/raw/U1mh8mcq'))()
...
local window = library:AddWindow("BRUTALITY HUB - BLADE BALL", {
	main_color = Color3.fromRGB(8, 128, 1),
	min_size = Vector2.new(400, 365),
	can_resize = false,
})

local features = window:AddTab("Combat")
features:Show()
features:AddLabel("Combat Section")

features:AddSwitch("Auto Parry",function(toggled)
    resolve_parry_Remote()
    getgenv().aura_Enabled = toggled
end)
features:AddSwitch("Hit Sound",function(toggled)
    getgenv().hit_sound_Enabled = toggle
end)
local features = window:AddTab("Misc")
features:AddLabel("Misc Section")

features:AddSwitch("Hit Effect",function(toggled)
    getgenv().hit_effect_Enabled = toggled
end)
features:AddSwitch("Kill Effect",function(toggled)
    getgenv().kill_effect_Enabled = toggled
end)
features:AddSwitch("Trail",function(toggled)
    getgenv().trail_Enabled = toggled
end)

local features = window:AddTab("Visual")
features:AddLabel("Visual Section")

features:AddButton("Visualizer",function()
...
local features = window:AddTab("Server")
features:AddLabel("Server Section")
...
local features = window:AddTab("Crates")
features:AddLabel("Crates Section")

features:AddButton("Buy Sword Crate",function()
    game:GetService("ReplicatedStorage").Remote.RemoteFunction:InvokeServer("PromptPurchaseCrate", workspace.Spawn.Crates.NormalSwordCrate)
end)
features:AddButton("Buy Explosion Crate",function()
    game:GetService("ReplicatedStorage").Remote.RemoteFunction:InvokeServer("PromptPurchaseCrate", workspace.Spawn.Crates.NormalExplosionCrate)
end)
features:AddSwitch("Auto Buy Sword Crate",function(state)
	getgenv().ASC = state
end)
features:AddSwitch("Auto Buy Explosion Crate",function(state)
    getgenv().AEC = state
end)
